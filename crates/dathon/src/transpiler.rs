...
//! transpiler bridges the two places `.dpy` source diverges from runnable
//! Python:
//!
//! 1. **Annotation atoms that wouldn't survive runtime evaluation.**
//!    dathon's atomic type names (`string`, `double`, `timestamp`, …) and
...
//!    runtime. The schema-cast is a dathon-only re-anchoring hint for the
...
//! to *something* at runtime — users import it from a dathon runtime
//! package or define a no-op base themselves (see `examples/dathon.py`).
...
//! keeps runtime tracebacks pointing at the right `.dpy` line.
...
///
/// Strips every dathon-only `.cast(DataFrame[Schema])` schema-cast, then
...
/// If `call` is dathon's fluent schema-cast — `<receiver>.cast(DataFrame[Schema])`
