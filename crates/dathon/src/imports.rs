...
//!
//! Each `.dpy` file gets its own resolution scope — schemas / functions
...
impl ModulePath {
    ...
    ///
    /// Returns the absolute `.dpy` path of the source module. The
    /// caller then matches it against the list of files dathon is
    /// checking.
...
mod tests {
    ...
    fn module_path_resolves_a_relative_import_to_a_sibling_file() {
        let importing = Path::new("/project/src/pipeline.dpy");
        ...
        assert_eq!(resolved, PathBuf::from("/project/src/schemas.dpy"));
    }
    ...
    fn module_path_walks_up_for_double_dot_relative_imports() {
        let importing = Path::new("/project/src/jobs/pipeline.dpy");
        let root = Path::new("/project");
        let module = ModulePath {
            level: 2,
            segments: vec!["schemas".to_string()],
    ...
    fn longest_common_ancestor_of_two_sibling_files_is_their_shared_dir() {
        let ancestor =
            longest_common_ancestor(["/project/src/a.dpy", "/project/src/b.dpy"]).unwrap();
    ...
    fn longest_common_ancestor_of_cousins_is_the_first_shared_dir() {
        let ancestor = longest_common_ancestor([
            "/project/src/jobs/pipeline.dpy",
            "/project/src/utils/schemas.dpy",
        ])
        .unwrap();
        assert_eq!(ancestor, PathBuf::from("/project/src"));
    }
