//! `CheckMode` — the strictness setting dathon shares with the embedded
//! Python engine. Drives which diagnostics surface.

use dathon::CheckMode;
...
#[test]
fn off_mode_silences_dathons_diagnostics() {
    let standard = dathon::check_with_mode("<t>.dpy", BAD_COLUMN, CheckMode::Standard);
    assert!(
        standard.has_code("D0030"),
        "the misspelled column should be an error at standard",
    );

    let off = dathon::check_with_mode("<t>.dpy", BAD_COLUMN, CheckMode::Off);
    assert!(
...
fn basic_and_strict_still_show_fundamental_errors() {
    for mode in [CheckMode::Basic, CheckMode::Strict] {
        let result = dathon::check_with_mode("<t>.dpy", BAD_COLUMN, mode);
