...
/// small pykrete-local enum rather than re-exporting `lsp_types`'s much
...
pub(crate) fn definition_with_scope<'a>(
    ...
    column: usize,
    focus_idx: usize,
    pyk_import_modules: &[(TextRange, usize)],
    schemas: &[Schema<'a>],
    registry: &Registry<'a>,
    ...
    // a project `.pyk` file → jump to the top of that file.
    for (range, target_idx) in pyk_import_modules {
