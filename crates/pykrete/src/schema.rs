...
//!
//! A pykrete schema is a Python class whose bases include `Schema`. The fields
...
/// and, when pykrete could work it out, its atomic type.
///
/// `ty` is `None` when the type couldn't be inferred (a function result
/// pykrete doesn't model, a column off another Derived schema, …). An
...
/// segment. A path pykrete can't verify — through an unknown type, or an
        ...
        for item in &dict.items {
            ...
            match field.resolve(schemas) {
                ...
                FieldResolution::NotABareName => errors.push((
                    "D0011",
                    format!(
                        "Column type '{}' is not a recognized pykrete type. \
...
mod tests {
    ...
    fn derived_schema_has_field_is_case_sensitive() {
        ...
        // case-insensitive matching, but pykrete is stricter; this keeps
