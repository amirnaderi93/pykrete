...
//! center around running `pykrete::check` against a small inline source
//! string and then asserting things about the resulting diagnostics.

#![allow(dead_code)]

use pykrete::CheckResult;

/// Run pykrete's analysis on a `.pyk` source string.
///
/// Tests write a small example as a raw string literal and pass it here.
/// The path embedded in diagnostics is `<test>.pyk` — purely cosmetic for
/// the tests, since we assert on codes and message contents, not on paths.
pub fn check(source: &str) -> CheckResult {
    pykrete::check("<test>.pyk", source)
}

/// Run pykrete's analysis at `strict` checking mode — surfaces the
...
pub fn check_strict(source: &str) -> CheckResult {
    pykrete::check_with_mode("<test>.pyk", source, pykrete::CheckMode::Strict)
}
