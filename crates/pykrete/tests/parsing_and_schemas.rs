...
#[test]
fn valid_python_with_no_pykrete_constructs_produces_no_diagnostics() {
...
fn class_without_Schema_base_is_not_recognized_even_with_annotations() {
    // A plain class with field annotations is NOT a pykrete schema.
...
fn d0010_fires_when_nested_class_does_not_extend_Schema() {
    ...
    // base it's not discovered as a pykrete schema, so the `nested: Foo`
...
fn deeply_nested_schemas_resolve_independently() {
    ...
    // pykrete doesn't need to walk through the chain.
    let result = check(
        r#"
class Inner(Schema):
    leaf: int
