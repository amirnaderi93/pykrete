...
#![allow(non_snake_case)] // PySpark / pykrete names (DataFrame, unionByName) appear in test names.

//!
//! Each test is a small, plausible-looking PySpark module written in pykrete
...
fn a_file_with_multiple_issues_reports_all_of_them() {
    ...
    // type mismatch. pykrete should surface all four independently.
    let result = check(
        r#"
class Orders(Schema):
    place_code: int
