//! The LSP multiplexer.
//!
//! pykrete-lsp embeds a Python language server (basedpyright) as a child
//! process and presents a single merged LSP to the editor. This module
//! owns:
//!
//! - [`DiagnosticStore`] — per-URI merge of pykrete's schema diagnostics
...
#[derive(Default)]
struct Sources {
    pykrete: Vec<Diagnostic>,
    child: Vec<Diagnostic>,
}

impl DiagnosticStore {
    /// Replace pykrete's diagnostics for `uri`. Returns the merged set
    /// the caller should publish.
    pub fn set_pykrete(&mut self, uri: Url, diagnostics: Vec<Diagnostic>) -> Vec<Diagnostic> {
        let entry = self.entries.entry(uri).or_default();
        entry.pykrete = diagnostics;
        Self::merge(entry)
    }
    ...
    fn merge(entry: &Sources) -> Vec<Diagnostic> {
        let mut out = Vec::with_capacity(entry.pykrete.len() + entry.child.len());
        out.extend(entry.pykrete.iter().cloned());
...
pub fn child_diagnostics_to_editor(params: &Value) -> Option<(Url, Vec<Diagnostic>)> {
    ...
    for d in raw {
        ...
        // apart from a pykrete one in the editor UI.
...
/// couldn't resolve, where the import targets another pykrete module —
/// i.e. a relative import (`from .schemas import …`). The engine
/// resolves imports as `.py`, so it never finds the sibling `.pyk`;
/// pykrete resolves those itself, so the engine's complaint there is
...
/// statement.
pub fn is_unresolved_pykrete_import(diagnostic: &Diagnostic, source: &str) -> bool {
...
/// awaiting the child's reply. pykrete's own answer is computed up front
...
pub struct PendingRequest {
    ...
    pub method: String,
    /// pykrete's own result for this request, already computed. `Null`
    /// when pykrete had nothing at the cursor.
    pub pykrete_result: Value,
...
///
/// `child` is `None` when no Python engine was found — pykrete-lsp then
/// runs pykrete-only, exactly the pre-multiplexer behavior.
pub struct Multiplexer {
    ...
    pub type_checking_mode: pykrete::CheckMode,
    ...
    /// gave the child (`pykrete-req-N`). Drained as the child answers.
    ...
    /// by the editor-facing id (`pykrete-c2e-N`). Drained as the editor
    /// answers.
...
impl Multiplexer {
    ...
    fn handshake(child: &mut ChildLsp, init_params: &Value) -> Option<Value> {
        ...
        let capabilities = loop {
            match child.receiver.recv() {
                Ok(msg) => {
                    if msg.get("id").and_then(|v| v.as_str()) == Some("pykrete-child-init") {
                        break msg
    ...
    /// Whether a Python engine is embedded. `false` means pykrete-only.
    ...
    /// document text into its virtual form (pykrete preamble + source).
    ...
    pub fn register_child_request(
        ...
        let editor_id = format!("pykrete-c2e-{}", self.next_child_request_seq);
    ...
    /// belongs to. `None` for an id pykrete-lsp never issued.
    ...
    ///
    /// `pykrete_result` is pykrete's own answer, computed by the caller
    /// up front. Returns `Some(pykrete_result)` when there's no child to
    ...
    pub fn forward_request(
        ...
        method: &str,
        params: Value,
        pykrete_result: Value,
        future_line: Option<u32>,
    ) -> Option<Value> {
        if self.child.is_none() {
            return Some(pykrete_result);
        }
        self.next_request_seq += 1;
        let child_id = format!("pykrete-req-{}", self.next_request_seq);
        self.send_to_child(&json!({
            "jsonrpc": "2.0",
            "id": child_id,
            "method": method,
        ...
        if self.child.is_none() {
            // The send failed and dropped the child — reply pykrete-only.
            return Some(pykrete_result);
        }
        self.pending.insert(
            child_id,
    ...
    /// so the caller can answer in-flight editor requests pykrete-only
    /// instead of leaving them to hang.
    pub fn drain_pending(&mut self) -> Vec<PendingRequest> {
        self.pending.drain().map(|(_, v)| v).collect()
    }

    /// Shut the child down — sent when pykrete-lsp itself exits.
    ...
    fn send_to_child(&mut self, msg: &Value) {
        if let Some(child) = self.child.as_mut() {
            ...
            // back to pykrete-only for the rest of the session.
...
///
/// pykrete-lsp implements **push** diagnostics (`publishDiagnostics`) —
...
/// editor, which would then pull from pykrete-lsp — a method pykrete-lsp
/// doesn't serve. Removing the capability keeps the child on the push
/// path pykrete-lsp actually handles.
...
/// Build the capability set pykrete-lsp advertises to the editor: its
/// own, plus the embedded engine's for the methods pykrete-lsp knows how
/// to proxy. A child capability is taken only when pykrete doesn't
/// already provide it (pykrete's own handler wins) and the method is on
/// the proxy allowlist.
///
...
pub fn merge_capabilities(mut pykrete: Value, child: &Value) -> Value {
    const PROXIED: [&str; 4] = [
    ...
    for key in PROXIED {
        if pykrete.get(key).is_none()
            && let Some(value) = child.get(key)
            && !value.is_null()
        {
            pykrete[key] = value.clone();
        }
    }
    if pykrete.get("semanticTokensProvider").is_none()
        && let Some(legend) = child.pointer("/semanticTokensProvider/legend")
    {
        pykrete["semanticTokensProvider"] = json!({ "legend": legend, "full": true });
    }
    pykrete
}
...
/// Python-analysis sections to `mode`. pykrete's own
/// `pykrete.typeCheckingMode` setting is authoritative for `.pyk` files,
...
fn set_type_checking_mode(settings: &mut Value, mode: pykrete::CheckMode) {
...
/// Merge pykrete's own answer for a fanned-out request with the child
/// engine's answer.
///
...
/// method just returns pykrete's result.
///
...
pub fn merge_child_response(method: &str, pykrete_result: Value, child_result: Value) -> Value {
    match method {
        "textDocument/hover" => merge_hover(pykrete_result, child_result),
        "textDocument/completion" => merge_completion(pykrete_result, child_result),
        "textDocument/definition" | "textDocument/references" => {
            merge_locations(pykrete_result, child_result)
        }
        "textDocument/signatureHelp" => child_result,
        "textDocument/documentHighlight" => remap_highlights(child_result),
        // `rename` / `prepareRename`: pykrete answers for a column (its
        // edit is already in editor coordinates — no remap); otherwise
        // the child answers and its result is remapped.
        "textDocument/rename" => {
            if pykrete_result.is_null() {
...
fn merge_hover(pykrete: Value, child: Value) -> Value {
    ...
    match (hover_text(&pykrete), hover_text(&child)) {
        ...
        (None, Some(_)) => child,
        (Some(d), Some(c)) => {
            // pykrete's hover carries no range; fall back to the child's
...
/// concatenated, pykrete's first; each child item's edit ranges are
...
/// list would also (wrongly) apply them to pykrete's items. Child items
/// that relied on a default edit range fall back to label insertion,
/// which is correct for the common `.`-member-access case.
fn merge_completion(pykrete: Value, child: Value) -> Value {
    let mut items = completion_items(&pykrete);
...
/// one `Location[]`. pykrete's schema-aware locations come first
...
/// against what's already there. For `references` pykrete contributes
...
fn merge_locations(pykrete: Value, child: Value) -> Value {
    let mut out = locations(&pykrete);
...
///
/// Only `.pyk` files are virtual documents carrying the injected
...
fn remap_location_to_editor(mut loc: Value) -> Option<Value> {
    let in_virtual_doc = loc
        ...
        .is_some_and(|uri| uri.ends_with(".pyk"));
    if !in_virtual_doc {
        return Some(loc);
    }
...
/// start position. Used to drop the duplicate when pykrete and the child
...
mod tests {
    ...
    fn uri() -> Url {
        Url::parse("file:///t.pyk").unwrap()
    }

    #[test]
    fn store_merges_pykrete_and_child_diagnostics() {
        let mut store = DiagnosticStore::default();
        let after_pykrete = store.set_pykrete(uri(), vec![diag(1, "D0030")]);
        assert_eq!(after_pykrete.len(), 1);
    ...
    fn store_replaces_per_source_not_appends() {
        let mut store = DiagnosticStore::default();
        store.set_pykrete(uri(), vec![diag(1, "first")]);
        let merged = store.set_pykrete(uri(), vec![diag(1, "second")]);
        // The second set_pykrete replaced the first, didn't append.
    ...
    fn store_clear_drops_everything_for_a_uri() {
        let mut store = DiagnosticStore::default();
        store.set_pykrete(uri(), vec![diag(1, "x")]);
        store.set_child(uri(), vec![diag(2, "y")]);
        store.clear(&uri());
        // After clear, a fresh pykrete set starts from empty.
        let merged = store.set_pykrete(uri(), vec![]);
    ...
    fn unresolved_relative_import_is_recognized_as_a_pykrete_import() {
        ...
        assert!(is_unresolved_pykrete_import(&relative, source));
    }
    ...
    #[test]
    fn non_import_diagnostic_is_never_treated_as_a_pykrete_import() {
        ...
        assert!(!is_unresolved_pykrete_import(&other, source));
    }
    ...
    fn request_params_without_a_position_pass_through_untouched() {
        let params = json!({ "textDocument": { "uri": "file:///t.pyk" } });
    ...
    fn merge_hover_stacks_both_sources_with_a_rule() {
        let merged = merge_child_response(
            "textDocument/hover",
            hover("**pykrete:** schema info"),
            hover("**python:** type info"),
        );
        let text = merged["contents"]["value"].as_str().expect("markdown");
        // pykrete's section comes first, then a horizontal rule, then
        // the Python engine's.
    ...
    fn merge_hover_falls_back_to_the_only_present_source() {
        // pykrete empty → the child's hover is returned as-is.
        let only_child =
            merge_child_response("textDocument/hover", Value::Null, hover("python only"));
        assert_eq!(only_child["contents"]["value"], json!("python only"));

        // child empty → pykrete's hover is returned as-is.
        let only_pykrete =
            merge_child_response("textDocument/hover", hover("pykrete only"), Value::Null);
        assert_eq!(only_pykrete["contents"]["value"], json!("pykrete only"));
    }
    ...
    fn hover_contents_flattens_a_marked_string_into_a_fence() {
        ...
        let merged = merge_child_response("textDocument/hover", hover("pykrete"), child);
        let text = merged["contents"]["value"].as_str().expect("markdown");
    ...
    fn test_multiplexer() -> Multiplexer {
        Multiplexer {
            child: None,
            diagnostics: DiagnosticStore::default(),
            type_checking_mode: pykrete::CheckMode::Standard,
    ...
    #[test]
    fn forward_request_with_no_child_replies_pykrete_only() {
        let mut mux = test_multiplexer();
        let pykrete_only = mux.forward_request(
            RequestId::from(1),
            "textDocument/hover",
            json!({}),
            hover("pykrete"),
        ...
        // No child → the caller gets pykrete's result straight back, and
        // nothing is parked in the pending table.
        assert_eq!(pykrete_only, Some(hover("pykrete")));
    ...
    fn take_child_request_returns_none_for_an_unissued_id() {
        let mut mux = test_multiplexer();
        assert!(mux.take_child_request("pykrete-c2e-999").is_none());
    }
    ...
    fn patch_engine_config_sets_type_checking_on_analysis_sections() {
        ...
        // back null and must carry pykrete's mode.
        let sections = vec!["python".to_string(), "python.analysis".to_string()];
        let patched =
            patch_engine_config(json!([null, null]), &sections, pykrete::CheckMode::Standard);
        assert_eq!(
    ...
    #[test]
    fn patch_engine_config_makes_pykretes_mode_authoritative() {
        // `pykrete.typeCheckingMode` drives the engine for `.pyk` files —
    ...
    #[test]
    fn merge_completion_concatenates_both_sources_pykrete_first() {
        let merged = merge_child_response(
    ...
    fn merge_definition_unions_locations_from_both_sources() {
        // pykrete's location is already in editor coordinates; the
        // child's is in another `.pyk` file at a distinct virtual line.
        let child = location("file:///other.pyk", virtualdoc::PREFIX_LINE_COUNT + 9);
        let merged = merge_child_response(
            "textDocument/definition",
            location("file:///t.pyk", 0),
            child,
        );
    ...
    fn merge_definition_coerces_a_location_link_to_a_location() {
        ...
        let merged = merge_child_response("textDocument/definition", Value::Null, child);
        let loc = &merged[0];
        assert_eq!(loc["uri"], json!("file:///t.pyk"));
    ...
    fn merge_definition_dedups_a_location_both_sources_resolve() {
        // pykrete and the child both resolve a Schema name to its class
        // declaration on real line 0 — the merged list has it once.
        let pykrete = location("file:///t.pyk", 0);
        let child = location("file:///t.pyk", virtualdoc::PREFIX_LINE_COUNT);
        let merged = merge_child_response("textDocument/definition", pykrete, child);
        assert_eq!(merged.as_array().expect("array").len(), 1);
    }

    #[test]
    fn merge_references_remaps_the_child_locations() {
        // `references` is a pure passthrough — pykrete contributes
        // nothing — but the child's locations still need remapping.
        let child = location("file:///t.pyk", virtualdoc::PREFIX_LINE_COUNT + 7);
    ...
    fn merge_capabilities_adopts_an_allowlisted_child_capability() {
        let pykrete = json!({ "hoverProvider": true });
        let child = json!({
            "signatureHelpProvider": { "triggerCharacters": ["(", ","] },
            "referencesProvider": true,
        });
        let merged = merge_capabilities(pykrete, &child);
        ...
        // pykrete's own capability is untouched.
    ...
    fn merge_capabilities_ignores_capabilities_off_the_allowlist() {
        ...
        // pykrete-lsp doesn't remap those results yet — they must not be
    ...
    #[test]
    fn merge_capabilities_keeps_pykretes_value_when_both_provide_a_method() {
        // pykrete already answers references itself (hypothetically) —
    ...
    #[test]
    fn merge_capabilities_with_no_child_leaves_pykretes_unchanged() {
        // No embedded engine → child capabilities are `Null`.
        let pykrete = json!({ "hoverProvider": true, "definitionProvider": true });
        let merged = merge_capabilities(pykrete.clone(), &Value::Null);
    ...
    fn merge_capabilities_restricts_semantic_tokens_to_full_with_the_legend() {
        // The child supports range and delta requests too; pykrete-lsp
    ...
    fn rename_remaps_document_changes_form() {
        let child = json!({
            "documentChanges": [{
                "textDocument": { "uri": "file:///t.pyk", "version": 1 },
                "edits": [
