...
//! calls [`build_project_snapshot`] to produce a list of every `.pyk`
//! file pykrete should see — every sibling file under the project root,
...
//! every analysis pass, which is fine at typical pykrete project sizes
//! (a few dozen `.pyk` files at most) and avoids the complexity of
...
pub fn build_project_snapshot(docs: &HashMap<Url, String>) -> Option<Vec<(String, String)>> {
    ...
    let mut pyk_paths: Vec<PathBuf> = Vec::new();
    if collect_pyk_paths(&project_root, &mut pyk_paths).is_err() {
    ...
    // discovered project root — for example, a one-off `.pyk` opened
    // by absolute path with no `pyproject.toml` nearby.
    for path in open_by_path.keys() {
        if !pyk_paths.contains(path) {
            pyk_paths.push(path.clone());
        }
    }
    pyk_paths.sort();
    pyk_paths.dedup();

    let mut out = Vec::with_capacity(pyk_paths.len());
    for path in pyk_paths {
...
/// Load the project's `pykrete.json` — searched for at the project root
/// and every ancestor. Absent or malformed → defaults, so the LSP keeps
/// working regardless.
pub fn load_config(docs: &HashMap<Url, String>) -> pykrete::Config {
...
mod tests {
    ...
    #[test]
    fn load_config_defaults_when_no_pykrete_json_is_present() {
        let root = tmpdir();
        let docs = docs_with(&root);
