...
//! when nothing is found the multiplexer runs pykrete-only.
...
impl ChildLsp {
    ...
    pub fn spawn(program: &str, args: &[String]) -> Option<ChildLsp> {
        ...
        let (tx, rx) = unbounded();
        thread::Builder::new()
            .name("pykrete-child-lsp-reader".to_string())
    ...
    /// doesn't exit on its own. Called when pykrete-lsp itself shuts down.
...
///    the VS Code extension bundles, or a `pykrete.pythonServer.path`
...
/// the multiplexer runs pykrete-only (no Python features), exactly the
