...
- **One-command v2.0 migration.** `DataFrame[X]` is a deprecated alias removed in v2.0. v1.6 ships `pykrete migrate` — it walks each binding's downstream usage, classifies it as Spark / pandas / ambiguous via call-graph adjudication, and rewrites the annotation in place to the dialect-tagged canonical name (atomic per file, token-preserving). Under `"typeCheckingMode": "strict"`, D0090 escalates from warning to error so strict-mode projects gate on the migration; non-strict modes keep the warning unchanged. See [Migrating to v2.0](https://amirnaderi93.github.io/pykrete/cookbook/#6-migrate-dataframex-to-the-v20-dialect-tagged-names) for the cookbook recipe.
