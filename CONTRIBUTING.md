...
1. **§9.2 retirement DONE in v1.16** (Cycle-0.1 #210) — the marker mechanism is retired. The cycle-0 chore rewrote `.github/workflows/extension-version-guard.yml` to gate bump-enforcement on the release-PR title (`chore(release):`, with the `chore/vX.Y-pr-f-*` head branch as a fallback) instead of the `.github/centralized-bump-cycle.marker` file, then deleted the marker. This resolved the v1.15 §1.i.3 chicken-and-egg (dropping marker creation in PR-S1 without first removing the workflow check fired the guard red on every mid-cycle PR touching `crates/pykrete/**`). The sequence that worked: cycle-0 = PR rewrites the guard to release-PR-title gating and deletes the marker, then PR-S1 ships normally with no marker to create.
...
The mechanism is release-PR-title gating: [`extension-version-guard.yml`](.github/workflows/extension-version-guard.yml) enforces the bump **only on the release PR** — identified by its `chore(release):` title (with the `chore/vX.Y-pr-f-*` head branch as a fallback signal). Every mid-cycle PR runs the same since-last-tag detection for logging but always passes, so a feature PR that touches `crates/pykrete{,-lsp}/**` is not flagged for lack of a bump. This replaced the v1.9–v1.15 `.github/centralized-bump-cycle.marker` file, retired in v1.16 (Cycle-0.1 #210).

**To cut a release**: in the release PR (titled `chore(release): ...` so the version-guard enforces the bump), bump `version` in the workspace `Cargo.toml` and `editors/vscode/package.json`, regenerate lockfiles, add a `CHANGELOG.md` entry, commit, merge, then `git tag vX.Y.Z && git push origin vX.Y.Z`. The [release workflow](.github/workflows/release.yml) builds binaries for macOS (arm64/x64) + Linux x64 + Windows (MSI), attaches the `.vsix`, publishes the extension to the Visual Studio Marketplace and Open VSX, and bumps the Homebrew tap — all automatically. See [`packaging/README.md`](packaging/README.md) for the full pipeline.
...
**The expected-failures allowlist (v1.16 PR-A1).** A gate fire that you cannot resolve in the PR you're holding — typically a roadmap header that only PR-G is allowed to move — can be held with an entry in `scripts/trust-claim-sweep-expected-failures.json`. The gate auto-discovers that file; `--expected-failures=FILE` overrides it. Each entry is `{ "surface": "<path>", "pattern": "<substring of the fire line, optional>", "reason": "<why deferred>", "expiresAfter": "<X.Y.Z>" }`.

`expiresAfter` is a **countdown, not a mute**. An entry is ACTIVE while `CURRENT_VERSION <= expiresAfter`: it suppresses its matching fire and logs `EXPECTED-FAILURE-SUPPRESSED`. Once `CURRENT_VERSION` **exceeds** `expiresAfter` the entry is STALE, and the gate fails loud with `EXPECTED-FAILURE-EXPIRED` — the fix is to resolve the underlying surface and delete the entry, never to push `expiresAfter` out another cycle. An active entry that matches no actual fire logs `EXPECTED-FAILURE-DEAD` (warning, not a failure) so entries don't accumulate after their surface is fixed by other means. The summary line reports `N fire(s); N suppressed; N surviving; N expired; N dead`; any surviving or expired count fails the gate.

The countdown resolves at cycle close. PR-G is the beat where the deferred surfaces get fixed and their entries deleted — after which `bash scripts/trust-claim-sweep-checklist.sh --current-version X.Y.Z` must pass with **zero** suppression entries. If the gate still fires, the content fix is incomplete; re-adding a suppression entry to make it green defeats the mechanism.
...
**Cycle-0 atomic snapshot rule (v1.16 backlog item 3).** The `pykrete-tests/README.md` corpus numbers and this repo's `scripts/trust-claim-sweep-checklist.snapshot.txt` MUST move in lockstep — never let one lag the other, or the backtick-preservation tripwire ends up guarding a number the README has already superseded. Whenever a cross-codebase batch (a pykrete-tests PR-P1) changes the probe / fixture / donor counts, the refresh chore MUST (1) update the `pykrete-tests/README.md` numbers to the canonical `python3 scripts/probes.py extract cross-codebase` values, (2) regenerate the snapshot here with `bash scripts/trust-claim-sweep-checklist.sh --snapshot` against that updated sibling checkout, and (3) land both in the same release-chore batch (the README in its pykrete-tests PR, the snapshot in the paired pykrete-repo chore — the post-tag catalog-pin chore is the usual home). Because the two files live in different repos, "atomic" means the same batch, not one commit. **PR-F does NOT refresh the snapshot when it only canonicalizes the pykrete-repo trust surfaces (`README.md`, docs-site) and leaves the `pykrete-tests/README.md` number refresh to the catalog-pin chore**: the committed snapshot still matches the un-refreshed sibling, so refreshing it to not-yet-published numbers would break the tripwire. Refresh the snapshot in the same chore that canonicalizes the sibling README.
