...
**D0080 constructor carve-out closed + `groupby.agg` Derived synthesis + `--compare-to` diff (new in v1.14).** D0080's last carve-out closes: `-> SparkFrame[X]` returning `pd.DataFrame({...})`, or `-> PandasFrame[X]` returning `spark.read.parquet(path)`, now fires D0080 — and `spark.createDataFrame(rows, schema=<Schema>)` fires when the dialect disagrees with the annotation. **Adopters with cross-dialect constructor returns at function boundaries will see new D0080 fires.** When both dialect and column-type mismatches land on the same return, the message is multi-clause (one fire, two clauses) — CI greps for "two D0080 fires at the same range" need to adjust. Pandas `df.groupby(...).agg("sum")` synthesizes a `Derived` envelope mirroring `pivot_table(aggfunc=)`: group keys plus the non-key columns at the aggregate-driven dtype (`count` / `nunique` → int64; `mean` / `std` / `var` / `median` → float64; `sum` / `min` / `max` / `first` / `last` preserve). v1.14 ships the single-string form; the dict and callable forms land in v1.16. The new `pykrete check --deprecation-report --compare-to=<prior.json>` flag emits a SIMPLE three-bucket diff (`added` / `removed` / `unchanged`) with exit-nonzero on `added`; mutually exclusive with `--ack` / `--snapshot` / `--fail-on-nonempty`. The envelope round-trips a v2 provenance pair (`pykreteSourceCommit` + `generatedAt`) so CI artifacts carry author-pinpointing metadata.

**Pandas chain-depth extension + synthesis-arm cross-codebase coverage closure + `resolve_override_ty` primitive (new in v1.15).** The v1.14 `groupby.agg` `Derived` envelope now survives one more transform: `pdf.groupby("k").agg("sum").reset_index(drop=True)` keeps the synthesized envelope alive, and `pdf.set_index([literal-keys])` removes the named literal-key columns from the accessible schema. **Adopters who incorrectly accessed non-aggregate columns after `groupby.agg().reset_index(drop=True)`, or who accessed literal keys after `set_index([keys])`, will see new D0030 fires** — align downstream code with the synthesized schema or access keys via `.index` (not modeled; `reset_index(drop=False)` index-as-column promotion is a v1.17 deferral). The v1.14 synthesis-arm cross-codebase coverage gaps close in pykrete-tests PR-P1 #50. The dtype-override family (shared between `pivot_table(aggfunc=)` and `groupby.agg`) consolidates behind the new `resolve_override_ty` primitive in preparation for the window-aggregation arms (the `Windowed` lattice variant was evaluated and rejected in v1.16 in favor of direct-chain recognition; deferred to v1.17).

**Time/window aggregation + dict & callable `groupby.agg` (new in v1.16).** `df.resample("<rule>").agg("<fn>")` now synthesizes a `Derived` schema instead of falling through — columns follow the aggregate-to-dtype table (`count` / `nunique` → Long; `mean` / `std` / `var` / `median` → Double; `sum` / `min` / `max` / `first` / `last` preserve the receiver column's dtype). `df.rolling(<n>).agg("<fn>")` aggregates numeric columns to Double, with aggfuncs restricted to `count` / `sum` / `mean` / `std` / `var` / `median` / `min` / `max`. `groupby.agg` gains its dict form (`{col: fn}`, which keeps **only** the named columns plus group keys) and its callable form (`np.mean`, `len`, which on an all-numeric frame keeps keys plus every non-key column at Unknown dtype). Only the single-expression `.agg("<str>")` spelling is recognized on the window chains — direct-method `df.resample("M").sum()`, held-intermediate chains, and non-string aggfuncs still fall through silently.

v1.16 also makes the checker quieter where pandas itself raises. Named-aggregation `df.groupby(k).agg(out=(col, fn))` was a **false positive** through v1.15 — it synthesized a keys-only schema and fired a bogus `returnColumnsMismatch` on correct code — and now declines to Unknown (precise modeling lands in v1.17). Numeric-restricting aggregation (`mean` / `std` / `var` / `median`, a `rolling` aggregation, or an opaque callable) over a frame with a non-numeric column also declines: pandas 2.x **raises** `TypeError` / `DataError` there rather than silently dropping the column the way pre-2.0 pandas did, so pykrete does not synthesize a schema for code that errors at runtime. And `reset_index(inplace=True)` / `set_index(inplace=True)` now resolve to `None`, matching pandas — **adopters who chained off an `inplace=True` result were relying on a schema pandas never returns**; drop `inplace=True` and rebind. **Adopters who accessed columns dropped by a dict-form `groupby.agg` will see new D0030 fires.**
