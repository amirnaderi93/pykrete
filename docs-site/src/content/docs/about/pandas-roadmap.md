...
description: The pandas-specific direction for pykrete — where v1.3 / v1.4 / v1.5 / v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11 / v1.12 landed, where v1.13+ is going, what v2.0 locks in.
---
...
## v1.13+ horizons (committed but unscheduled)

- **D0080 dialect-on-return checker arm**: v1.12 PR-P1 #42 closed the cross-codebase trust coverage gap; the checker still compares column names + shared-column types without comparing the Dialect tag, so `-> SparkFrame[X]` returning `.toPandas()` body fires zero diagnostics. Carve-out from the v1.12 trust closure.
- **Richer `pivot_table` aggfunc-driven inference**: v1.12 PR-D1 ships the 11-string allowlist classifier as scaffolding (recognition is informational); v1.13+ consumes the classifier output so the result schema knows `aggfunc="count"` produces int64 columns vs `aggfunc="mean"` produces float64.
- **Full `pivot_table` / `melt` / `stack` / `unstack` schema-tracking** — the wide / long output schemas (variable column values become column names of the result frame for `pivot_table`; `var_name` / `value_name` become columns of the long frame for `melt`; index-level promotion / demotion for `stack` / `unstack`). v1.6 + v1.7 + v1.10 + v1.11 ship literal-form column checking on the inputs; the output-shape models are paired with the rest of pandas reshape in v1.13+.
- **`.loc` non-literal forms**: `.loc[mask, "col"]` (boolean mask) row-key tracking, `.loc[:, "a":"b"]` (column range), and `pdf.iloc[...]` — carried forward to v1.13 paired with broader pandas reshape.
...
- **Broader pandas method modeling**: `df.groupby(...).agg(...)`, `df.reset_index`, `df.set_index` (v1.10 shipped `stack`; v1.11 shipped `unstack`). Currently fall through to opaque.
...
- **Retrofitting pandas `PROBE-TYPE-IS` to the v1.3 hybrid donors** (MLflow, Feast, iceberg-python) — v1.4 / v1.5 / v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11 / v1.12 deliberately scoped these out; revisit in v1.13+.
...
- **polars support**: separate dialect with separate idioms. Tracked for v1.13+ if user demand surfaces; not gated on pandas work, but realistically follows pandas reshape.
...
| v1.11.0 | "pandas `df.unstack(level=, fill_value=)` literal-form (mirror of v1.10 `stack`); cross-codebase property probes for the v1.10 D0091 8-property surface (pykrete-tests PR-P1 #39); audit-tooling block (trust-claim sweep checklist + CHANGELOG cite-check + auto-label workflow); LSP test tempdir-per-test isolation; walker polish (mixed-indent + decorator-with-comment + counter-semantics)" | yes — `271 probes` total across `17 donors` |
| v1.12.0 | "auto-label workflow dispatches `release-gate.yml` via `actions.createWorkflowDispatch` (closes v1.11 GITHUB_TOKEN calendared promise); D0080 returnTypeMismatch cross-codebase trust coverage (closes longest-standing gap since v1.6); `pivot_table(aggfunc=)` 11-string allowlist recognition (informational; primes v1.13+ aggfunc-driven inference); multi-line ack-marker rationale block (spec §6.1.4); release-gate `cargo test` memoization via `PYKRETE_TESTS_COUNT_FILE`; LSP tempdir parent-dir RAII guard" | yes — `279 probes` total across `17 donors` |
| v1.13.0 (target) | + "D0080 dialect-on-return checker arm; rest of pandas reshape (`groupby.agg` + full `pivot_table` / `melt` / `stack` / `unstack` output schema-tracking); `.loc` non-literal forms + `.iloc`; `.query` / `.eval` mini-DSLs; richer `pivot_table` aggfunc-driven inference (consumes v1.12 allowlist classifier); `--include-py` / `--changed-only` / `--compare-to` migrate flags" | TBD per spec |
