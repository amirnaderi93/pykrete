...
Of the cross-dialect closures shipped in v1.5, `.toPandas()` (PR-A1) and `.loc[:, "col"]` (PR-C, via the yfinance probe pair added in [pykrete-tests #24](https://github.com/amirnaderi93/pykrete-tests/pull/24)) have load-bearing cross-codebase probe coverage. PR-A2's `createDataFrame(pdf)` Gate (b), PR-A3's pandas `.head/.tail/.first` chain-survival, PR-B1's cross-DataFrame `df.select(df_other["col"])` routing, and PR-B2's `column_name_arg` DataFrame-binding gate ship with unit-test coverage only; the v1.6 probe batch lands cross-codebase exercise for each.
