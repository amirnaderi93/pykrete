...
**New in v1.7: migrator `--check` default + pandas `melt` literal-form + `dialect_signals` shared module + Spark-D1 audit closure.** v1.6 shipped `pykrete migrate` as a rewrite-in-place default; v1.7 flips that to `--check` (preview verdicts on stdout; exit 1 if any site needs attention) and adds `--apply` as the opt-in for the in-place rewrite. A first-run on v1.7 with no flag emits a one-line stderr warning so adopters discover the change without reading release notes. The pandas `melt` literal-form arm ships as the v1.7 reshape downpayment: `pdf.melt(id_vars=["a", "b"], value_vars=["c", "d"], var_name="variable", value_name="value")` resolves string-literal arguments against `PandasFrame[X]`'s schema, firing D0030 on a typo with a *did you mean*. The dispatch is gated on `receiver_is_pandas_inherited` so the existing Spark `melt`/`unpivot` arm's behavior on `SparkFrame[X]` receivers is unchanged. v1.6's parallel `PANDAS_ONLY_SIGNALS` / `PANDAS_INHERITED_ARMS` lists with a "Keep in sync" comment between them (the v1.6 architecture-audit Important #3 finding) collapse into a shared `crates/pykrete/src/dialect_signals.rs` module — paired with a CI-guard test pinning the `expr.rs` pandas-arm methods to the shared list (honest scoping: catches "added to one list, forgot the other" — the parallel-edit drift class; does NOT catch "added a wholly new dispatched arm and updated neither list" — the omitted-edit class deferred to v1.8). 14 Spark-only methods get added to `SPARK_DISCRIMINATORS` (`selectExpr`, `freqItems`, `approxQuantile`, `crosstab`, `colRegex`, `summary`, `mapInPandas`, `mapInArrow`, `writeTo`, `writeStream`, `unpivot`, `rdd`, `isStreaming`, `sparkSession`) — `corr` / `cov` deliberately excluded for pandas collision risk. `pykrete migrate` parse-error skips now surface on stderr; CRLF marker normalization lands for Windows source files. Internal audit-debt mop-up: dead `_source: &str` param dropped from `ambiguous_site_offsets` / `has_ambiguous_in_file`; the two-vector lockstep loop in the migrate parse-error filter collapses to single-pass.
...
What the v1.7 probe suite does **not** yet verify, all tracked for v1.8+:

- **Full `pivot_table` and `melt` output schema-tracking** — the wide / long output schemas (variable column values become column names for `pivot_table`; `var_name` / `value_name` become columns of the long frame for `melt`). v1.6 + v1.7 ship literal-form column checking on the inputs; the output-shape models are paired with the rest of pandas reshape in v1.8.
- **`.loc[mask, "col"]` (boolean mask) row-key tracking, `.loc[:, "a":"b"]` (column range), `pdf.iloc[...]`** — deferred to v1.8, paired with broader pandas reshape.
...
- **Broader pandas method modeling** (`groupby.agg`, `stack` / `unstack`, `reset_index`, `set_index`).
...
The v1.7 probe batch ([pykrete-tests PR-P1](https://github.com/amirnaderi93/pykrete-tests/pull/27)) added 6 negative probes for the previously-uncovered D0040 / D0050 / D0051 D-codes (2 per code), lifting cross-codebase coverage from 241 to 247. The coordinated `melt` probe pair ([pykrete-tests PR-D1](https://github.com/amirnaderi93/pykrete-tests/pull/28), positive on a pandas-heavy donor + negative for the typo-in-`value_vars` shape) is open and awaits the v1.7.0 catalog pin bump.
