...
description: How pykrete is tested against `171 fixtures` and `312 probes` from `17 donors` — what we cover, what the goldens and probes verify, what's deliberately out of scope.
---

[pykrete-tests](https://github.com/amirnaderi93/pykrete-tests) is a separate repo that vendors fixtures from `17 donors` covering widely-used PySpark and pandas codebases, adds pykrete annotations the way a real adopter would, and on every push runs two release-blocking suites against pykrete built fresh from a pinned source-commit: a golden-diff suite (JSON output of `pykrete check` against a frozen snapshot) and a `312`-probe schema-tracking suite (inline `# PROBE-*` assertions that columns survive transforms, that specific diagnostics fire on deliberately-corrupted fixtures, that Spark and pandas type-tracking hold through transformations, and that the v1.3 pandas check sites — `PandasFrame[X]` column refs, the six dispatched operations, the deprecated-`DataFrame[X]` warning — work end-to-end). It exists for two reasons:
...
`171 fixtures` (58 annotated + 113 deliberately-corrupted under `probes_negative/`) across `17 donors`. The 10 PySpark donors are all Apache 2.0; the 10 pandas-coverage donors carry Apache 2.0, BSD-3-Clause, or MIT licenses (verified per donor at the pinned tag).
...
| **delta** | [delta-io/delta](https://github.com/delta-io/delta) | 4 | 8 |
| **kedro-plugins** | [kedro-org/kedro-plugins](https://github.com/kedro-org/kedro-plugins) | 3 | 1 |
| **iceberg-python** | [apache/iceberg-python](https://github.com/apache/iceberg-python) | 3 | 4 |
| **hudi** | [apache/hudi](https://github.com/apache/hudi) | 3 | 3 |
| **mlflow** | [mlflow/mlflow](https://github.com/mlflow/mlflow) | 6 | 9 |
| **feast** | [feast-dev/feast](https://github.com/feast-dev/feast) | 4 | 4 |
| **quinn** | [MrPowers/quinn](https://github.com/MrPowers/quinn) | 3 | 2 |
| **dbt-spark** | [dbt-labs/dbt-spark](https://github.com/dbt-labs/dbt-spark) | 2 | 8 |
...
|---|---|---|---:|---:|
| **mlflow** | hybrid | (see above) | 6 (incl. v1.3 `pandas_dataset.pyk`) | 9 |
| **feast** | hybrid | (see above) | 4 (incl. v1.3 `pandas_entity_df.pyk`) | 4 |
| **iceberg-python** | hybrid | (see above) | 3 (incl. v1.3 `pandas_score_dataset.pyk`) | 4 |
| **scikit-learn** | canonical-fixture-only | [scikit-learn/scikit-learn](https://github.com/scikit-learn/scikit-learn) | 1 | 5 |
| **statsmodels** | canonical-fixture-only | [statsmodels/statsmodels](https://github.com/statsmodels/statsmodels) | 1 | 3 |
| **pandera** | canonical-fixture-only | [unionai-oss/pandera](https://github.com/unionai-oss/pandera) | 1 | 9 |
...
| **prophet** | direct-dispatch | [facebook/prophet](https://github.com/facebook/prophet) | 1 | 3 |
...
- **Full `melt` / `stack` / `unstack` output schema-tracking** — the wide / long output schemas. v1.6 + v1.7 ship literal-form column checks on the inputs; v1.10 + v1.11 add `stack` / `unstack` literal-form on the input; v1.13 ships pivot_table `Derived` synthesis; v1.14 ships `groupby.agg` Derived synthesis; v1.15 ships `reset_index(drop=True)` + `set_index([literal-keys])` chain-depth pass-through; remaining output-shape modeling is paired with the rest of pandas reshape in v1.17+.
...
- **Broader pandas method modeling** (`reset_index(drop=False)` index-as-column promotion, `set_index(<expr>)` non-literal forms, `expanding.agg`, named-aggregation `groupby(k).agg(out=(col, fn))` — `stack` shipped in v1.10, `unstack` in v1.11, `pivot_table(aggfunc=)` Derived synthesis in v1.13, `groupby.agg` Derived synthesis in v1.14, `reset_index(drop=True)` + `set_index([literal-keys])` literal-form chain-depth in v1.15, `resample.agg` + `rolling.agg` + dict-form and callable `groupby.agg` in v1.16).
- **The v1.16 window-aggregation arms, cross-codebase.** `resample.agg`, `rolling.agg`, and the dict-form / callable `groupby.agg` shapes ship checked by in-crate unit tests only; no donor fixture exercises them. Donor coverage is tracked for v1.17.
...
- **`PROBE-TYPE-IS` synth-shape coverage beyond D0081 (Spark side).** The current synth shape (`{df}.select(col("x") + 1)`) falsifies on non-numeric. D0082 (`crossTypeComparison`) still needs its own synth shape; the raw-mutation suite covers it until then. (D0080 `returnTypeMismatch` cross-codebase coverage shipped in v1.12 per pykrete-tests PR-P1 #42; v1.13 ships the dialect-on-return checker arm; v1.14 closes the constructor carve-out; v1.15 closes the v1.14 synthesis-arm cross-codebase coverage gap per pykrete-tests PR-P1 #50.)
...
v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11 / v1.12 / v1.13 / v1.14 / v1.15 / v1.16 CLI / checker features (`pykrete migrate` and its `--check`-default / `--apply` flip, `--report-aliases`, `--deprecation-report` v1 + v2 envelopes, `--ack` filter, `--snapshot=<path>`, `--fail-on-nonempty`, `--compare-to=<prior.json>` SIMPLE three-bucket snapshot diff, call-graph adjudication, parse-error surface, CRLF marker normalization, multi-line ack-marker rationale block per spec §6.1.4, `pivot_table(aggfunc=)` 11-string allowlist classifier, the v1.13 D0080 dialect-on-return checker arm, the v1.13 `pivot_table(aggfunc=)` Derived-schema synthesis, the v1.14 D0080 constructor-arm carve-out closure with multi-clause format, the v1.14 `groupby.agg` Derived synthesis, the v1.14 envelope schema v2 provenance pair `pykreteSourceCommit` + `generatedAt`, the v1.15 `reset_index(drop=True)` + `set_index([literal-keys])` chain-depth arms, the v1.15 `resolve_override_ty` primitive consolidating the dtype-override family, and the v1.16 `resample.agg` / `rolling.agg` window-aggregation arms plus the dict-form and callable `groupby.agg` arms and the `inplace=` guards) are verified by synthetic integration tests in `crates/pykrete/tests/`, not by the cross-codebase probe suite.

Diagnostics without dedicated cross-codebase coverage (covered by synthetic tests instead): D0080 `returnTypeMismatch` was in this list through v1.11 — the longest-standing trust gap since v1.6 — and got cross-codebase probe coverage in v1.12 per pykrete-tests PR-P1 #42. D0091 `crossDialectMethodMismatch` was in this list through v1.8 and got cross-codebase probe coverage in v1.9 (2 negative probes per pykrete-tests PR-P1 #32 on pandera + delta), extended in v1.10 per pykrete-tests PR-P1 #34 with 6 more strict-mode / bare-attribute / shape-changes probes; the v1.10 8-property surface expansion (`na`, `write`, `writeStream`, `storageLevel`, `index`, `values`, `shape`, `T`) gets cross-codebase property probes shipped in v1.11 per pykrete-tests PR-P1 #39. D0073 `transformInputMismatch`, D0083 `nullabilityMismatch` were in this list through v1.7 and got cross-codebase probe coverage in v1.8 (2 negative probes each per pykrete-tests PR-P1 #30). D0040 `unionSchemaMismatch`, D0050 `returnColumnsMismatch`, D0051 `argumentColumnsMismatch` were in this list through v1.6 and got cross-codebase probe coverage in v1.7 (2 negative probes each, per pykrete-tests PR-P1 #27).
