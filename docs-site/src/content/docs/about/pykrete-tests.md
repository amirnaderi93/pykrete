...
description: How pykrete is tested against `158 fixtures` and `299 probes` from `17 donors` — what we cover, what the goldens and probes verify, what's deliberately out of scope.
---

[pykrete-tests](https://github.com/amirnaderi93/pykrete-tests) is a separate repo that vendors fixtures from `17 donors` covering widely-used PySpark and pandas codebases, adds pykrete annotations the way a real adopter would, and on every push runs two release-blocking suites against pykrete built fresh from a pinned source-commit: a golden-diff suite (JSON output of `pykrete check` against a frozen snapshot) and a `299`-probe schema-tracking suite (inline `# PROBE-*` assertions that columns survive transforms, that specific diagnostics fire on deliberately-corrupted fixtures, that Spark and pandas type-tracking hold through transformations, and that the v1.3 pandas check sites — `PandasFrame[X]` column refs, the six dispatched operations, the deprecated-`DataFrame[X]` warning — work end-to-end). It exists for two reasons:
...
`158 fixtures` (52 annotated + 106 deliberately-corrupted under `probes_negative/`) across `17 donors`. The 10 PySpark donors are all Apache 2.0; the 10 pandas-coverage donors carry Apache 2.0, BSD-3-Clause, or MIT licenses (verified per donor at the pinned tag).
...
What the v1.14 probe suite does *not* yet verify, all tracked for v1.15+:
...
- **Broader pandas method modeling** (`reset_index`, `set_index`, `resample.agg`, `rolling.agg`, `expanding.agg` — `stack` shipped in v1.10, `unstack` in v1.11, `pivot_table(aggfunc=)` Derived synthesis in v1.13, `groupby.agg` Derived synthesis in v1.14).
...
- **`PROBE-TYPE-IS` synth-shape coverage beyond D0081 (Spark side).** The current synth shape (`{df}.select(col("x") + 1)`) falsifies on non-numeric. D0082 (`crossTypeComparison`) still needs its own synth shape; the raw-mutation suite covers it until then. (D0080 `returnTypeMismatch` cross-codebase coverage shipped in v1.12 per pykrete-tests PR-P1 #42; v1.13 ships the dialect-on-return checker arm; v1.14 closes the constructor carve-out — the matching cross-codebase negative probes are tracked for v1.15+.)
- **D0080 constructor-arm cross-codebase coverage** (a fixture with `-> SparkFrame[X]` returning `pd.DataFrame({...})` or similar mismatched constructor) — v1.14 ships the checker arm closing the carve-out; the corresponding cross-codebase negative probes are tracked for v1.15+.
- **`groupby.agg` Derived-synthesis cross-codebase coverage** (asserting `out[...]` access against the Derived envelope on a real-library fixture) — v1.14 ships the synthesis arm; the cross-codebase property probes are tracked for v1.15+.
