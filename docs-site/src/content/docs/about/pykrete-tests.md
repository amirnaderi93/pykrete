...
description: How pykrete is tested against `164 fixtures` and `305 probes` from `17 donors` — what we cover, what the goldens and probes verify, what's deliberately out of scope.
---

[pykrete-tests](https://github.com/amirnaderi93/pykrete-tests) is a separate repo that vendors fixtures from `17 donors` covering widely-used PySpark and pandas codebases, adds pykrete annotations the way a real adopter would, and on every push runs two release-blocking suites against pykrete built fresh from a pinned source-commit: a golden-diff suite (JSON output of `pykrete check` against a frozen snapshot) and a `305`-probe schema-tracking suite (inline `# PROBE-*` assertions that columns survive transforms, that specific diagnostics fire on deliberately-corrupted fixtures, that Spark and pandas type-tracking hold through transformations, and that the v1.3 pandas check sites — `PandasFrame[X]` column refs, the six dispatched operations, the deprecated-`DataFrame[X]` warning — work end-to-end). It exists for two reasons:
...
`164 fixtures` (54 annotated + 110 deliberately-corrupted under `probes_negative/`) across `17 donors`. The 10 PySpark donors are all Apache 2.0; the 10 pandas-coverage donors carry Apache 2.0, BSD-3-Clause, or MIT licenses (verified per donor at the pinned tag).
...
What the v1.15 probe suite does *not* yet verify, all tracked for v1.16+:
...
- **Full `melt` / `stack` / `unstack` output schema-tracking** — the wide / long output schemas. v1.6 + v1.7 ship literal-form column checks on the inputs; v1.10 + v1.11 add `stack` / `unstack` literal-form on the input; v1.13 ships pivot_table `Derived` synthesis; v1.14 ships `groupby.agg` Derived synthesis; v1.15 ships `reset_index(drop=True)` + `set_index([literal-keys])` chain-depth pass-through; remaining output-shape modeling is paired with the rest of pandas reshape in v1.16+.
- **`.loc[mask, "col"]` (boolean mask) row-key tracking, `.loc[:, "a":"b"]` (column range), `pdf.iloc[...]`** — carried forward to v1.16+, paired with broader pandas reshape.
...
- **Broader pandas method modeling** (`reset_index(drop=False)` index-as-column promotion, `set_index(<expr>)` non-literal forms, `resample.agg`, `rolling.agg`, `expanding.agg` — `stack` shipped in v1.10, `unstack` in v1.11, `pivot_table(aggfunc=)` Derived synthesis in v1.13, `groupby.agg` Derived synthesis in v1.14, `reset_index(drop=True)` + `set_index([literal-keys])` literal-form chain-depth in v1.15).
