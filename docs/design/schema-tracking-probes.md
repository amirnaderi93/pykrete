...
1. **Scheduled out-of-bump drift workflow** (cross-codebase repo).
   ...
   open a tracking issue (not a PR — the actual sync still needs human
   review). This catches the "main grew a D-code, our pin is behind"
   case before it festers into a bad release.
...
1. **New: `scripts/probes.py`** (~300 LOC, stdlib-only, Python 3.10+;
   ...
   - `verify(fixture_path, normalized_actual_json) -> list[ProbeFailure]`:
     ...
     - `TYPE-IS`: implemented via the probe-to-diagnostic translation
       described in "Golden format" above (synthetic D0083 target on
       a scratch fixture copy). Individual probe instances drop out
...
8. **New: `.github/workflows/catalog-drift-watch.yml`** — scheduled
   ...
   `pykrete-core@main` and opens a tracking issue on mismatch. Not
   release-blocking; it surfaces drift early so the next
...
17. **Out-of-bump catalog drift surfacing.** The scheduled
    ...
    growing a D-code before our `PYKRETE_REF` pin moves. Open
    follow-up: does it open a GitHub issue, post to a slack-equivalent
    channel, or also gate the next `PYKRETE_REF` bump PR? Recommendation:
    open a GitHub issue (low-friction, auditable, doesn't depend on
    chat). Gating the bump PR is overkill; the bump PR's own CI runs
    the (non-scheduled) drift checker which will catch any actual
    staleness. Also: does the issue auto-close when the catalog is
    refreshed, or stay open for a human to close? Recommendation:
    auto-close via a follow-up workflow step that re-runs the diff
    and closes if clean — keeps the issue queue tidy.
...
- **Type pass** (~half day). Add `PROBE-TYPE-IS` on selected bindings
  using the synthesizer rewrite (D0083 target). Focused on bindings
...
synthesizer rewrite that only needs to target one D-code (D0083), the
budget tightens back to the original headline.
