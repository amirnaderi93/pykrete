...
- **Trust-claim surfaces** swept end-to-end for v1.5 reality: README "Reliability and trust", docs-site `about/production-readiness`, `about/pykrete-tests`, the splash page, the docs-site roadmap, the canonical roadmap, and the pandas-roadmap all refresh to **227 probes across 86 fixtures from 17 donors** (cross-dialect handoff added to the "what we verify" list; the pykrete-tests v1.5 cycle ships incremental probe coverage on top of the v1.4 baseline of 223 probes / 83 fixtures).
...
- **Column resolution** through the Spark v1.0 surface plus the pandas analogues — 150 positive probes across 45 annotated fixtures.
- **Diagnostic firing** on broken fixtures — 46 negative probes across 41 `probes_negative/` fixtures pinning D0030 `unknownColumn`, D0060 `missingJoinKey`, D0081 `nonNumericArithmetic`, D0082 `crossTypeComparison`, D0084 `enumValueMismatch`, and D0090 `deprecatedDataFrameAlias`.
...
- **Pandas type tracking** through dispatched chains (shipped v1.4) on `PandasFrame[X]`, scoped to D0081 via the assign-arithmetic synth — 24 `PROBE-TYPE-IS` markers across the pandas-coverage donors.
